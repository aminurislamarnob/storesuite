<?php
/**
 * StoreSuite product form: ACF `time_picker` field input.
 *
 * Native time input; converted to ACF's H:i:s on save.
 *
...
$input_value = DateFormats::to_input( 'time_picker', $value );

if ( '' === $input_value && ! $is_edit_mode && ! empty( $field['default_to_current_date'] ) ) {
	$input_value = DateFormats::now_for_input( 'time_picker' );
}
?>
<input
	type="time"
