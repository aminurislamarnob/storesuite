...
- **Core services:**
  ...
  - `Integration/YoastSeoIntegration.php` — when Yoast SEO is active (`WPSEO_VERSION`), renders the tabbed "SEO (Yoast)" card (`templates/products/product-seo-yoast.php`, script `assets/frontend/product-seo.js`) on the product form via the generic `storesuite_product_form_after_main_cards` action, and saves its own whitelisted `storesuite_yoast_*` fields to `_yoast_wpseo_*` meta on `storesuite_new_product_added` / `storesuite_product_updated`. A hidden `storesuite_yoast_seo` marker guards against saves that did not come from the card (Quick Edit shares that save path). Advanced fields follow Yoast's rules (`can_edit_advanced()`, filter `storesuite_yoast_can_edit_advanced`); social fields follow Yoast's `opengraph` / `twitter` toggles. Yoast touch points are protected methods so tests run without Yoast
  - `Product/ProductAI.php` — AI product copy generation through the WP core AI Client. Fields come from a registry (`get_fields()`, extended by the `storesuite_ai_text_fields` filter; each definition declares label, output sanitizing, instruction and its setting, on/off setting, prompt intro, context need, insert `target`, rows, optional `length`, optional `prompt_lines` callable, optional settings `group`). `storesuite_ai_text_generator` supplies a callable that replaces the core client and makes text AI count as available — the seam PHPUnit and the e2e fake (`tests/pw/bin/storesuite-e2e-fake-ai.php`) use. `get_settings_groups()` feeds integration blocks on the AI settings page; `SettingsController` derives AI settings keys from the registry. `assets/frontend/product-ai.js` inserts by the button's `data-target` and posts any `[data-ai-context]` field with each request
